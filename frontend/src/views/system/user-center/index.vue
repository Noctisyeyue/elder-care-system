...
const form = reactive<ProfileUpdateForm>({ realName: '', phone: '', gender: '' })
...
async function loadProfile() {
  const [profileData, avatar] = await Promise.all([getProfile(), getUserAvatar().catch(() => avatarUrl.value)])
  profile.value = profileData; avatarUrl.value = (avatar as string) || avatarUrl.value
  Object.assign(form, { realName: profileData.realName || '', phone: profileData.phone || '', gender: profileData.gender || '' })
}
...
async function handleAvatarChange(e: Event) { const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return; if (!['image/jpeg', 'image/png'].includes(file.type)) { ElMessage.error('请上传 JPG 或 PNG 格式图片'); return }; if (file.size / 1024 / 1024 > 2) { ElMessage.error('图片大小不能超过 2MB'); return }; const formData = new FormData(); formData.append('file', file); avatarUrl.value = await uploadAvatar(formData); ElMessage.success('头像更新成功') }

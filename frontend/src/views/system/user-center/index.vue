    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :md="8">
        <el-card class="profile-card" shadow="never">
          ...
          <div class="profile-body">
            ...
            />
            <div v-else class="text-user-avatar user-avatar" :style="textAvatar.style">
              {{ textAvatar.initial }}
            </div>
            ...
            <el-button type="primary" plain class="avatar-btn" @click="triggerAvatarUpload">
              更换头像
            </el-button>
            <input
              ref="avatarInput"
      ...
      <el-col :xs="24" :md="16">
        ...
        <el-card class="common-card section-card" shadow="never">
          ...
          <p class="card-desc">
            邮箱用于登录、找回密码和接收验证码。修改邮箱需要验证新邮箱，修改成功后需要重新登录。
          </p>
    ...
    >
      <div class="email-dialog-content">
        <div class="current-email-tip">
          当前邮箱：<span class="email-text">{{ profile.email || '未绑定' }}</span>
        </div>
        <el-form
          ref="emailFormRef"
          :model="emailForm"
...
const textAvatar = computed(() =>
  getTextAvatar(profile.value.realName || userStore.realName || profile.value.email),
)
...
.common-card :deep(.el-card__header) {
  padding: 16px 20px;
